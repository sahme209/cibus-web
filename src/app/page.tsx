...
export default function HomePage() {
  ...
  const filteredRestaurants = useMemo(() => {
    ...
    if (filters.priceLow) list = list.filter((r) => r.minimumOrder < 500);
    if (filters.priceHigh) list = list.filter((r) => r.minimumOrder >= 1000);
  ...
  return (
    <div className="animate-page-enter" style={{ background: "var(--bg-secondary)" }}>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mt-6">
        <Link
          href="/search"
          className="flex items-center gap-4 rounded-2xl p-4 transition-all hover:-translate-y-0.5 hover:shadow-md"
          style={{ background: "var(--bg-card)", boxShadow: "var(--shadow-sm)", border: "1px solid var(--border-subtle)" }}
        >
          <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0" style={{ background: "rgba(100,149,237,0.1)" }}>
            <svg className="w-6 h-6" style={{ color: "#6495ED" }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold" style={{ color: "var(--text-primary)" }}>Schedule for Later</p>
            <p className="text-xs" style={{ color: "var(--text-tertiary)" }}>Plan ahead — pick a time that works for you</p>
          </div>
